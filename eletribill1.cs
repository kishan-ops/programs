...
namespace ConsoleApplication4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("enter customer type:\n press for 1 industry\n press for 2 residential\n press for 3 service oriented organization\n press for 4 temple and endowment\n press for 5 ariculture\n");
            byte ctype = byte.Parse(Console.ReadLine());
            double totbill1 =cunits;
            switch (ctype)
            {
                case 1:
                    if (cunits < 300||cunits>=300)
                    {
                        if (cunits <= 100)
                        
                            Console.WriteLine("total bill is:" + cunits * 8);
                        
                        if (cunits > 100 && cunits <= 150)
                        
                            Console.WriteLine("total bill is:" + cunits * 10);
                        
                        if (cunits > 150 && cunits <= 200)
                        
                            Console.WriteLine("total bill is:" + cunits * 15);
                        
                        if (cunits > 200 && cunits <= 300)
                        
                            Console.WriteLine("total bill is:" + cunits * 20);
                        
                        if (cunits > 300)
                        
                            Console.WriteLine("total bill is:" + cunits * 30);
